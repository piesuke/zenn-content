...
設定後は `Tab` で候補が表示されるようになり、`Ctrl + Space` でも同じ候補リストを呼び出せます。矢印キーで候補を選んで `Enter` を押すだけなので、マウスに手を伸ばさなくて済むのも嬉しいところです。


### 補完機能でできること

普段よく使う `git` や `pnpm` のサブコマンドが自動的に学習され、複雑なサブコマンド入力も数文字で呼び出せるようになります。以下のスクリーンショットでは、`git` の履歴からブランチ名まで含めて提案してくれています。
